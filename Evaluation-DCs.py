...
parser.add_argument('--dclf_dir',type=str,help='the path ot new dictionary')
...
parser.add_argument('--results',type=str,help='path of input files under initial_data')
parser.add_argument('--customcache',type=str,default='../mutatedPLMs',help='customodel, if here, the plm is replaced..')
parser.add_argument('--type',type=str,default='sst',choices=['sst','mrpc'],help='help to match the dc model')
...
if os.path.exists(args.bugs)==True:
    ...
    for Mutate_Type in list(ContrastSet.keys()):
        ...
        if args.type=='sst':
            if Mutate_Type not in ['synon_contr', 'SenWordFuzz', 'swapGender_sentiSynonymSubstitution']:
                continue
                
        if args.type=='mrpc':
            if Mutate_Type not in ['mrpc_fuzz']:
                continue
        ...
        embeddings=[]
        for unit in Data:
            embeddings.append(feature_extraction(unit[0:3]))
