...
for thres in [args.thres]:
    ...
    for i in range(len(plms)):
        ...
        mm = os.system("python Fixing.py --alldata {} --bugs {} --plm {} --gpu {} --output {} --thres {} --tokencache {}".format(contrast_set_path, os.path.join(bug_dirs,plm.replace('/','-')),plm,gpu_id, os.path.join(fixmodel_dir,plm.replace('/','-')),thres,dis_cache_dir))
        logger.info('Fixing bugs : {}'.format(mm>>8))

        
        mm = os.system("python Testing-general.py --contrastset {} --plm {} --gpu {} --outputdir {} --norm {} --thres {} --tokencache {} --customodel {} --customcache {}".format(contrast_set_path,plm, gpu_id, new_bug_dirs, args.norm, thres, dis_cache_dir, plm.replace('/','-'), fixmodel_dir))
        logger.info('Test fixed bugs : {}'.format(mm>>8))


        
        mm = os.system("python TrainDCs.py --dataset {} --validation {} --gpu {} --plm {} --output_dir {} --customodel {} --customcache {}".format(train_data_sst,valid_data_sst, gpu_id, plms[i], new_dc_dirs_sst,plm.replace('/','-'), fixmodel_dir))
        logger.info('Train fixed dcs on sst :{}'.format(mm>>8))

        mm = os.system("python TrainDCs.py --dataset {} --validation {} --gpu {} --plm {} --output_dir {} --customodel {} --customcache {}".format(train_data_mrpc,valid_data_mrpc, gpu_id, plms[i], new_dc_dirs_mrpc,plm.replace('/','-'), fixmodel_dir))
        logger.info('Train fixed dcs on mrpc :{}'.format(mm>>8))

        
        mm =os.system("python Evaluation-DCs.py --contrastset {} --plm {} --gpu {} --bugs {}  --results {}  --dclf_dir {} --type {} --customodel {} --customcache {}".format(contrast_set_path,plm, gpu_id, os.path.join(new_bug_dirs,plm.replace('/','-')) ,new_bug_evals_path, new_dc_dirs_sst,'sst',plm.replace('/','-'), fixmodel_dir))
        logger.info('Eval fixed dcs on sst :{}'.format(mm>>8))
        mm = os.system("python Evaluation-DCs.py --contrastset {} --plm {} --gpu {} --bugs {}  --results {}  --dclf_dir {} --type {} --customodel {} --customcache {}".format(contrast_set_path,plm, gpu_id, os.path.join(new_bug_dirs,plm.replace('/','-')) ,new_bug_evals_path, new_dc_dirs_sst,'mrpc',plm.replace('/','-'), fixmodel_dir))
